---
title: "Profil"
---

# Mohamed Haddad

> **Ingénieur d'État en Réseaux et Cybersécurité** | Spécialisé en infrastructures Zero Trust, micro-segmentation et environnements haute disponibilité.

Bienvenue sur mon espace technique. J'évolue à l'intersection des opérations de sécurité (SecOps) et de l'ingénierie d'infrastructure, avec un focus sur la conception d'architectures résilientes et l'intégration de solutions Cloud-Native et Bare-Metal.

### 🛠️ Stack Technique & Expertise

*   **Infrastructure & Virtualisation :** Proxmox VE (Cluster HA), Docker, TrueNAS, Linux.
*   **Réseaux & Sécurité :** pfSense, Micro-segmentation (VLAN 802.1Q), Architecture Zero Trust, Apache Guacamole.
*   **Sécurité Offensive & Défensive :** OpenVAS, ELK Stack, Analyse de malwares (Flare VM), Nmap.
*   **Développement & IA :** Python, architecture RAG (Llama 3, ChromaDB) pour la cybersécurité.

### 📌 Engagement

Actuel **Team Leader** au Career Boot Camp 2026 et membre actif de la communauté de cybersécurité **B-Secure** (2021-2024), je documente ici mes déploiements techniques, mes architectures de laboratoires (Cyber Ranges) et mes outils de Threat Intelligence.

### 📬 Contact & Réseaux

*   **Email :** [contact@mhaddad-sec.me](mailto:contact@mhaddad-sec.me)
*   **LinkedIn :** [linkedin.com/in/mhaddad-sec](https://linkedin.com/in/mhaddad-sec/)
*   **GitHub :** [github.com/hadadsimo978-max](https://github.com/hadadsimo978-max)